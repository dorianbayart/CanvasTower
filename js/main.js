...
var Tower = {
  ...
  drawRange: function(ctx) {
    ctx.beginPath();
    ctx.arc(Math.round(this.x), Math.round(this.y), Math.round(this.range), 0, Math.PI*2, false);
    ctx.closePath();
    ctx.strokeStyle = this.color;
    ctx.stroke();
  }
...
function drawUI() {
  ...
  if(dateUI1 - dateUI0 > delayUI) {
    ...
    for (var i = 0; i < towers.length; i++) {
      ...
      if(mouseDistance < towers[i].radius) {
        towers[i].drawRange(ctxUI);
      }
    }
